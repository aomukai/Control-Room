...
public final class OutlineParser {
    ...
    public static OutlineDocument parseOutlineFromSourceContent(String raw, OutlineDocument base) {
        String normalized = normalizeNewlines(raw == null ? "" : raw);
    ...
    private static List<OutlineScene> buildScenes(List<SceneSegment> segments, OutlineDocument base) {
    ...
    private static TitleAndBody resolveTitleAndBody(SceneSegment segment) {
    ...
    private static ParseCandidate selectBestCandidate(List<String> lines) {
    ...
    private static ParseCandidate buildCandidate(String method, List<SceneSegment> segments, double bias) {
    ...
    private static CandidateStats computeStats(List<SceneSegment> segments) {
    ...
    private static double computeScore(CandidateStats stats, double bias) {
    ...
    private static List<SceneSegment> buildHeadingSegments(List<String> lines) {
    ...
    private static List<SceneSegment> buildLabelSegments(List<String> lines) {
    ...
    private static List<SceneSegment> buildDividerSegments(List<String> lines) {
        List<SceneSegment> segments = new ArrayList<>();
    ...
    private static List<SceneSegment> buildParagraphSegments(List<String> lines) {
        List<SceneSegment> segments = new ArrayList<>();
        List<String> buffer = new ArrayList<>();
    ...
    private static String extractDocumentTitle(List<String> lines) {
        for (String line : lines) {
            String trimmed = line.trim();
            if (trimmed.isEmpty()) {
                continue;
    ...
    private static String extractHeadingText(String headingLine) {
        if (headingLine == null) {
            return null;
        }
    ...
    private static String extractHeadingTextFromBody(String body) {
        if (body == null) {
            return null;
        }
    ...
    private static String stripLeadingHeading(String body) {
        if (body == null) {
            return "";
        }
        String[] lines = body.split("\\n", -1);
    ...
    private static String fingerprint(String title, String body) {
